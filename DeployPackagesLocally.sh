...
mkdir -pv $target && cp -v $PACKAGEDIR/Dolittle.Interaction.WebAssembly.Core.2.1000.0.nupkg $target
# Unzip package
tar -xzf $PACKAGEDIR/Dolittle.Interaction.WebAssembly.Core.2.1000.0.nupkg -C $target
# Create an empty .sha512 file, or else it won't recognize the package for some reason
touch $target/Dolittle.Interaction.WebAssembly.Core.2.1000.0.nupkg.sha512
