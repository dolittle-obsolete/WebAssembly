...
export function configure(aurelia: Aurelia, config: any) {
    ...
    if (config.entryPoint === '') throw new Error('Missing entrypoint in Dolittle WebAssembly plugin configuration');
    if (config.assemblies.length === 0) throw new Error('Missing assemblies in Dolittle WebAssembly plugin configuration');

    console.log(`Using '${config.entryPoint}' as entrypoint for WebAssembly `);

    storage.preload()
        .then(_ => EventProcessorOffsetRepository.preload())
        .then(_ => EventStore.preload()).catch((error: any) => {
            ...
            (window as any)._mongoDB = new MongoDB();
            (window as any)._eventStore = {
