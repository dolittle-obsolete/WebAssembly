...
{
    /// <summary>
    /// Represents a <see cref="IMongoCollection{T}"/> for mini mongo in the browser.
    /// </summary>
    /// <typeparam name="T">Type of document the collection is for.</typeparam>
    public class MiniMongoCollection<T> : IMongoCollection<T>
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="MiniMongoCollection{T}"/> class.
        /// </summary>
        /// <param name="collectionName">Name of collection.</param>
        /// <param name="configurationFor"><see cref="Configuration"/> object.</param>
        ...
        /// <inheritdoc/>
        public async Task<ReplaceOneResult> ReplaceOneAsync(FilterDefinition<T> filter, T replacement, UpdateOptions options = null, CancellationToken cancellationToken = default)
        {
            await _jsRuntime.Invoke<object>($"{MiniMongoDatabase._globalObject}.database.{_collectionName}.upsert", replacement).ConfigureAwait(false);
            return new ReplaceOneResult.Acknowledged(1, 1, BsonValue.Create(0));
        }

        /// <inheritdoc/>
        public async Task<ReplaceOneResult> ReplaceOneAsync(IClientSessionHandle session, FilterDefinition<T> filter, T replacement, UpdateOptions options = null, CancellationToken cancellationToken = default)
        {
            await _jsRuntime.Invoke<object>($"{MiniMongoDatabase._globalObject}.database.{_collectionName}.upsert", replacement).ConfigureAwait(false);
            return new ReplaceOneResult.Acknowledged(1, 1, BsonValue.Create(0));
        }
        ...
        {
            var filterAsJson = filter.ToJson();
            await _jsRuntime.Invoke<object>($"{MiniMongoDatabase._globalObject}.database.{_collectionName}.deleteOne", filterAsJson).ConfigureAwait(false);
